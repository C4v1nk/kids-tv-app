// =====================================================================
// KIDS TV — CLOUDFLARE WORKER (site + video relay, all in one)
// =====================================================================
// WHY THIS FILE EXISTS (read this before touching anything):
//
// The very first attempt to move this project to Cloudflare put the video
// relay in functions/watch-video.js, which is the right file in *classic*
// Cloudflare Pages. But when this project was created through Cloudflare's
// "Connect to Git" flow, Cloudflare quietly built it as a newer kind of
// project called "Workers with static assets" instead of classic Pages —
// same dashboard, different engine underneath. That engine does NOT look
// at a functions/ folder at all, so functions/watch-video.js was never
// actually being run, even though it was sitting right there in GitHub.
// (You can still see the old file in the repo — it's harmless, just dead
// weight now. This file replaces the job it was trying to do.)
//
// This file is the one and only thing Cloudflare actually runs. It does
// two jobs:
//   1. If the request is for /watch-video (the video relay teachers'
//      TVs quietly use as a last resort), it does the exact same Google
//      Drive lookup functions/watch-video.js used to do.
//   2. For every other request (index.html, and anything else), it just
//      hands the request to Cloudflare's built-in static file server —
//      env.ASSETS.fetch(request) — which is the same thing that would
//      have happened automatically with no Worker at all.
//
// In plain terms: nothing about how the dashboard looks or works changes.
// This file only exists so the /watch-video safety net actually turns on.
// =====================================================================
...
// If a Google API key is available (set it in Cloudflare under Settings →
// Variables and Secrets, named GOOGLE_API_KEY), skip the whole business of
// reading Google's warning page and just ask Google's official API for the
// file. Optional — with no key set, everything below works exactly as if
// this function had never heard of it.
...
async function handleWatchVideo(request, env) {
...
// This is the one function Cloudflare actually calls for every request that
// reaches this Worker. Cloudflare tries to match a real static file (like
// index.html) FIRST and only calls this function when nothing matched — so
// in practice, this only ever runs for /watch-video, since that path isn't
// a real file. The env.ASSETS.fetch(request) fallback below exists so that,
// even if that ever changes, nothing else breaks: any request this Worker
// doesn't specifically recognize is simply handed to the normal static file
// server, exactly as if this file didn't exist.
export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);

    if (url.pathname === "/watch-video") {
      return handleWatchVideo(request, env);
    }
